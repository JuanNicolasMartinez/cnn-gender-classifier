...
export function LiveCaptureStage({
  ...
  return (
    <section className="clinical-card-static relative overflow-hidden rounded-[var(--radius-lg)] p-0">
      <div className="relative aspect-[16/10] w-full bg-[var(--color-card-dark)] xl:aspect-[16/9]">
        {previewUrl ? (
          <Image
            src={previewUrl}
        ...
        {mounted && !previewUrl && !cameraActive ? (
          <div className="pointer-events-none absolute inset-0 grid place-items-center">
            <div className="rounded-[var(--radius-md)] border border-[rgba(247,247,240,0.4)] bg-[rgba(20,24,23,0.55)] px-5 py-3 text-center text-sm leading-6 text-[var(--color-white-warm)] backdrop-blur-md">
              {cameraSupported
                ? "La cámara está inactiva. Inícíala desde el panel izquierdo."
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-4 flex flex-wrap items-center justify-center gap-2 px-4 md:bottom-6">
          <button
            type="button"
            className="pointer-events-auto inline-flex items-center justify-center gap-2 rounded-full border border-[var(--color-accent)] bg-accent px-5 py-3 text-sm font-semibold text-primary transition-transform duration-200 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-55"
          ...
          <div className="glass-pill pointer-events-auto flex max-w-[18rem] items-center truncate px-4 py-3 text-sm leading-6 text-secondary">
            {selectedFile
        ...
        {cameraError ? (
          <div className="absolute inset-x-4 bottom-20 mx-auto max-w-md rounded-[var(--radius-md)] border border-[var(--color-danger-border)] bg-[var(--color-clinical-pink-soft)] px-4 py-3 text-sm leading-6 text-[var(--color-danger)] md:bottom-24">
            {cameraError}
          </div>
        ) : null}
      </div>
    ...
    <aside className="absolute left-3 top-3 hidden w-[260px] rounded-[var(--radius-md)] border border-[rgba(143,145,142,0.42)] bg-[rgba(247,247,240,0.78)] p-4 shadow-[0_18px_40px_-18px_rgba(0,0,0,0.45)] backdrop-blur-md sm:block md:left-5 md:top-5 md:w-[280px]">
      ...
      <ol className="mt-4 space-y-2 text-sm leading-5 text-secondary">
        <li className="flex gap-2">
          ...
          Centra tu rostro en la silueta.
        </li>
        <li className="flex gap-2">
        ...
        <li className="flex gap-2">
          ...
          Pulsa Capturar para enviar el frame.
        </li>
      </ol>
      ...
      <button
        type="button"
        className={cn(
          "glass-pill mt-3 inline-flex w-full items-center justify-center gap-2 px-3 py-2 text-xs font-medium",
    ...
    <aside className="absolute right-3 top-3 hidden w-[260px] rounded-[var(--radius-md)] border border-[rgba(143,145,142,0.42)] bg-[rgba(247,247,240,0.78)] p-4 shadow-[0_18px_40px_-18px_rgba(0,0,0,0.45)] backdrop-blur-md sm:block md:right-5 md:top-5 md:w-[280px]">
      ...
      >
        ...
        <p className="text-xs leading-5 text-secondary">
          o haz click para explorar. Threshold productivo {threshold.toFixed(2)}.
        </p>
        <input
          type="file"
      ...
      <p className="mt-3 text-[0.66rem] uppercase tracking-[0.22em] text-secondary">
      ...
      {selectedFile ? (
        <p className="glass-pill mt-3 truncate px-3 py-2 text-xs text-secondary">
          Lista: {selectedFile.name}
        </p>
      ) : null}
    </aside>
